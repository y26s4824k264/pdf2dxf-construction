...
rc23 修复多字库短横冲突引起的漏识别：字体先经多字库扫描确认，再以同一来源文字行的三个不同汉字锚点复核完整字形。十个字库同时加载的 84 个 DXF 样例完整恢复由 62 增至 64；逐字体指定对应字库的 84 个 PDF 仍有 82 个完整恢复。未知和冲突轮廓继续保留，已有 `r2` 字库无需重下。详见 [验证记录](docs/VALIDATION.md)。
